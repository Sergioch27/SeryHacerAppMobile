...
const CartDetails = () => {
  ...
  useEffect(() => {
    ...
    const updateRemaining = () => {
      const nextExpiration = cartItems.reduce((closest, item) => {
        const expiration = item?.expirationDate ? new Date(item.expirationDate).getTime() : null;
      ...
      const diff = nextExpiration ? Math.max(nextExpiration - Date.now(), 0) : 0;
      setRemainingMs(diff);
    };
  ...
  const handleRemoveReservation = (item, reservation) => {
    const confirmationText = reservation.isBlock
      ? '¿Estás seguro de eliminar este bloque completo de 4 horas?'
      : '¿Estás seguro de eliminar esta hora de la reserva?';
  ...
  const handleCheckout = () => {
    ...
    try {
      setSubmitting(true);
      dispatch(clearCartError());
      await dispatch(startReservationCheckout({ cartKey: cartItems[0].cartKey })).unwrap();
      Alert.alert('Pago iniciado', 'Se abriÃ³ Transbank para completar el pago.');
    } catch (error) {
      Alert.alert('No se pudo finalizar la compra', 'Revisa la configuración de la orden o intenta nuevamente.');
    } finally {
      setSubmitting(false);
    }
  };
